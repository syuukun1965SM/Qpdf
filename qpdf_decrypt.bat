...
rem 機能概要: パスワードの設定されているpdfファイルからパスワードを解除するバッチファイル

rem 引数に対象ファイルを指定するのもOK
rem 対象pdfファイルが設定されたらファイル自体の有無確認を行う
rem 対象pdfファイルが存在したらパスワード設定状況を確認する
rem パスワード設定されていなかったらその旨表示して処理の継続・終了の判断を仰ぐ
rem パスワードが未設定だったらパスワード入力を要求する
