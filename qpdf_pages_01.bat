...
rem 機能概要: --pagesオプションを用いた各種機能選択用バッチファイル"qpdf_pages.bat"から選択肢[1]を選定したときにコールされるサブモジュール。
...
:PAGE_SELECTION
rem 抽出ページ選択処理部
set /p SEL_Pages="抽出するページ範囲(例: 1,2,3,5-10)・除外ページ範囲(例:x6,7)を指定してください:  "
...
rem echo 入力ファイル名は %In_File% です。
rem echo 指定されたページ範囲は %SEL_Pages_Arr% です。
rem echo 指定されたページ範囲を抽出し、%Out_File%に出力します。
rem echo.
rem pause
qpdf.exe %In_File% --pages . %SEL_Pages_Arr% -- %Out_File%
